...
def print_trace_summary(label: str, trace: RunTrace) -> None:
...
async def main() -> None:
    agent = AgentLoop(model=DemoModel(), tools=[ExternalWaitTool()])

    paused = await agent.run([Message.user_text("start external job")])
    ...
    resume_agent = AgentLoop(model=DemoModel(), tools=[ExternalWaitTool()])

    resumed = await resume_agent.run_snapshot(
        ResumeInput(
            snapshot=restored_snapshot,
            append_messages=[Message.user_text("job-1 completed successfully")],
