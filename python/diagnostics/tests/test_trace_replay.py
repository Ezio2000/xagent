...
def test_run_trace_from_events_normalizes_missing_model_error_retryable() -> None:
    trace = RunTrace.from_events(
        "run-1",
        [
            AgentEvent(
...
def test_replay_diagnostic_mode_handles_non_trace_input() -> None:
    diagnostic = replay_trace(object(), strict=False)

    assert diagnostic.valid is False
    assert diagnostic.steps == 0
