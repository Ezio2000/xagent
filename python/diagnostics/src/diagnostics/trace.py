...
from kernel import AgentEvent, AgentState, AgentStatus, EventTypes, ModelErrorInfo, ResumeInput

from diagnostics._frozen import freeze_value, thaw_value
...
def replay_trace(trace: Any, *, strict: bool = True) -> ReplayResult:
    """Validate a trace without calling models or tools."""

    try:
        trace_payload = _trace_payload(trace)
        canonical = RunTrace.from_dict(trace_payload)
        result = _ReplayValidator(canonical).validate()
    except (AttributeError, ReplayError, TypeError, ValueError, KeyError) as exc:
        replay_error = exc if isinstance(exc, ReplayError) else ReplayError(str(exc))
        if strict:
            raise replay_error from exc
        return ReplayResult(False, _trace_step_count(trace), message=str(replay_error))
...
def _trace_payload(trace: Any) -> Mapping[str, Any]:
    if isinstance(trace, Mapping):
        return cast(Mapping[str, Any], trace)
    return _expect_mapping(trace.to_dict(), "trace")


def _trace_step_count(trace: Any) -> int:
    try:
        payload = _trace_payload(trace)
        return len(_expect_sequence(payload.get("steps", ()), "trace steps"))
