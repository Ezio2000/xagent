...
`support` package for deterministic fakes and fixtures, but the normative inputs are the fixtures,
...
- Provide the standard conformance tools: `echo`, `accept`, `handoff`, `fail`,
  ...
  `python/conformance/src/conformance/_standard_tools.py`; support tool
  fixtures are separate controlled components and must not define conformance
  semantics.
